...
def load_transformers(env) -> List[Transformer]:
    ...
    if env.charging_network_topology:
        ...
        cs_counter = 0
        for i, tr_data in enumerate(env.charging_network_topology):
            cs_ids = []
            for _ in tr_data['charging_stations']:
                cs_ids.append(cs_counter)
                cs_counter += 1
            transformer = Transformer(id=i,
                                      env=env,
                                      cs_ids=cs_ids,
                                      max_power=tr_data['max_power'],
...
def load_ev_charger_profiles(env) -> List[EV_Charger]:
    ...
    if env.charging_network_topology:
        ...
        cs_counter = 0
        for i, tr_data in enumerate(env.charging_network_topology):
            for cs_data in tr_data['charging_stations']:
                if not v2g_enabled:
                    cs_data['max_discharge_current'] = 0
                    cs_data['min_discharge_current'] = 0

                ev_charger = EV_Charger(id=cs_counter,
                                        connected_bus=0,
                                        connected_transformer=i,
                                        min_charge_current=cs_data['min_charge_current'],
                                        max_charge_current=cs_data['max_charge_current'],
                                        min_discharge_current=cs_data['min_discharge_current'],
                                        max_discharge_current=cs_data['max_discharge_current'],
                                        voltage=cs_data['voltage'],
                                        n_ports=cs_data['n_ports'],
                                        phases=cs_data['phases'],
