...
def get_statistics(env) -> Dict:
    ...
    transformer_limit = np.mean([tr.max_power for tr in env.transformers])
    if transformer_limit > 0:
        peak_transformer_loading_pct = (np.max(env.current_power_usage) / transformer_limit) * 100
        average_transformer_loading_pct = (np.mean(env.current_power_usage) / transformer_limit) * 100
    else:
        peak_transformer_loading_pct = 0
        average_transformer_loading_pct = 0
